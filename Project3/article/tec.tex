...
The Higgs boson was the last missing piece, and was discovered in 2012 by both ATLAS\footnote{Paper from ATLAS collaboration can be found \href{https://arxiv.org/abs/1207.7214}{here}.} and CMS\footnote{Paper from CMS collaboration can be found \href{https://arxiv.org/abs/1207.7235}{here}.} at CERN. There has however not been any new physics discoveries beyond the standard model so far, and this is somewhat alarming. The standard model has several issues, some of them includes lack of an explanation of dark energy, cosmic inflation, the hierarchy problem and not including gravity. \par 
...
\section{Theory}
Some of theory sections about anomaly detection and machine learning algorithms are based on earlier work done in other courses, such as \cite{FYSSTK} and \cite{ML_PROJ}.
...
For the purpose of this report we will mostly consider collective anomalies. Anomalies in high energy physics have to be collective to claim anything, as noise and other factors can and often do create point or event collective anomalies. Through triggers\footnote{ATLAS experiment \href{https://atlas.cern/Discover/Detector/Trigger-DAQ}{website}.}, one hopes that most of the noise and other factors are accounted for, but it is not a perfect system.\par 

In high energy physics we can, using machine learning, separate anomaly detection into two categories, supervised and unsupervised searches. Figure \ref{fig:corr_class}, \ref{fig:wrong_class} and \ref{fig:unsup_class} are examples that are not based on actual predictions and are only meant as illustrations.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.49\textwidth]{figures/theory/correct_class.pdf}
    \caption{Supervised classification of anomaly where the target model is correct. Here the machine learning model manages to correctly identify the anomalies. }
...
%Some features contain angular information, and thus should be treated differently than other features. Assume an event has 315 degrees as some angular feature, and the auto encoder reconstructs it as 2 degrees. This is not as bad a reconstruction as if we only look at the difference between the two values as numbers. Some proposed solutions to periodic feature handling has been put forward by Str{\"u}mke and Maeland\cite{10.1007/978-981-15-1960-4_10} and could also affect the performance of the auto encoder.  \par \par
...
\begin{table}[h]
    ...
    nbjet70 & int & Number of b-jets with with given machine learning output b-tag certainty \\ \hline
    nbjet77 & int & Number of b-jets with  with given machine learning output b-tag certainty  \\ \hline
    nbjet85 & int & Number of b-jets with with given machine learning output b-tag certainty   \\ \hline
